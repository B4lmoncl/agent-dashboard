...
export function FlavorToast({ toast, onClose }: { toast: { message: string; icon: string; sub?: string }; onClose: () => void }) {
  ...
  return (
    ...
    >
      {toast.icon.startsWith("/") ? <img src={toast.icon} alt="" style={{width:28,height:28,imageRendering:"smooth"}} className="flex-shrink-0" /> : <span className="text-2xl flex-shrink-0">{toast.icon}</span>}
      <div className="flex-1 min-w-0">
