...
export default function GuildHallBackground() {
  ...
  const mtRef      = useRef<HTMLDivElement>(null);   // mountains layer
  const ghRef      = useRef<HTMLDivElement>(null);   // guild-hall layer

  // Parallax via direct DOM — no React re-renders on scroll
  useEffect(() => {
    const onScroll = () => {
      const sy = window.scrollY;
      if (mtRef.current) mtRef.current.style.transform = `translateY(${sy * 0.012}px)`;
      if (ghRef.current) ghRef.current.style.transform = `translateY(${sy * 0.028}px)`;
    };
  ...
  // Guild hall: day = 06:00–17:59, night = 18:00–05:59
  const h = typeof window !== "undefined" ? new Date().getHours() : 12;
  const isGuildDay = h >= 6 && h < 18;

  // Mountains opacity: slightly dimmer at night
  const mtOpacity = 0.70;
  // Guild hall opacity: subtle, content stays readable
  const ghOpacity = 0.20;
  ...
  return (
    <>
      ...
      {/* Layer 3 — Guild Hall PNG (day / night crossfade) */}
      <div
        ref={ghRef}
        style={{
          position: "fixed",
      ...
      >
        ...
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/images/bg-guildhall-day-v2.png"
          alt=""
          style={{
            display: "block",
            position: "absolute",
            ...
            height: "auto",
            imageRendering: "pixelated",
            opacity: isGuildDay ? ghOpacity : 0,
        ...
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src="/images/bg-guildhall-night-v2.png"
          alt=""
          style={{
            display: "block",
            width: "100%",
            height: "auto",
            imageRendering: "pixelated",
            opacity: isGuildDay ? 0 : ghOpacity,
            transition: "opacity 2s ease",
            userSelect: "none",
          }}
        />
