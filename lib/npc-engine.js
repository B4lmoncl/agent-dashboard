...
function spawnNpc(giver, now) {
  ...
  // First pass: generate all IDs so we can wire prevQuestId/nextQuestId
  const generatedIds = chain.map(() => `quest-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`);
  ...
  for (let idx = 0; idx < chain.length; idx++) {
    ...
    const quest = {
      id: generatedIds[idx],
      title: resolved.title || qt.title || `${giver.name}'s Quest`,
      ...
      prevQuestId: idx > 0 ? generatedIds[idx - 1] : null,
      nextQuestId: idx < chain.length - 1 ? generatedIds[idx + 1] : null,
      rarity: giver.rarity,
...
function trySpawnNpcs(now) {
  ...
  // Guaranteed NPC visits (from gacha pulls) — spawn first, before random
  for (const u of Object.values(state.users)) {
    if (!u.guaranteedNpcVisit) continue;
    const npcId = u.guaranteedNpcVisit;
    if (activeIds.has(npcId)) { delete u.guaranteedNpcVisit; continue; } // already active
    const giver = state.npcGivers.givers.find(g => g.id === npcId);
    if (!giver || !giver.portrait) { delete u.guaranteedNpcVisit; continue; }
    if (state.npcState.activeNpcs.length + spawned >= NPC_MAX_ACTIVE) break;
    spawnNpc(giver, now);
    activeIds.add(npcId);
    spawned++;
    delete u.guaranteedNpcVisit;
    console.log(`[npc] Guaranteed visit: ${giver.name} (from gacha) for ${u.name}`);
  }

  for (let slot = 0; slot < maxSpawnThisCycle; slot++) {
    if (state.npcState.activeNpcs.length + spawned >= NPC_MAX_ACTIVE) break;
    const baseChance = slot === 0 ? 0.45 : 0.15;
    ...
    const candidates = state.npcGivers.givers.filter(g => {
      ...
      if (!g.portrait) return false; // Don't spawn NPCs without portrait images
...
function forceSpawnMinimumNpc(now) {
  ...
  if (!candidates.length) {
    // Fallback: allow any non-permanent NPC that's not active and not on cooldown
    const fallback = state.npcGivers.givers.filter(g => {
      if (NPC_PERMANENT_IDS.has(g.id)) return false;
      if (activeIds.has(g.id)) return false;
      if (!g.portrait) return false; // Don't spawn NPCs without portrait images
      const cooldownUntil = state.npcState.cooldowns[g.id];
...
function checkCompanionQuestTimeLimits() {
  ...
  for (const q of state.quests) {
    ...
    const hourPart = berlinParts.find(p => p.type === 'hour');
    const bH = hourPart ? parseInt(hourPart.value, 10) : 12;
...
function checkPeriodicTasks() {
  const now = new Date();
  // Process any NPC departures that have passed since last check
  processNpcDepartures(now);
  checkCompanionQuestTimeLimits();
  // World Boss auto-spawn check
  try {
    const { checkAutoSpawn } = require('../routes/world-boss');
    checkAutoSpawn();
  } catch { /* world-boss module not loaded yet */ }
