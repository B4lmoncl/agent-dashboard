...
function updateUserStreak(userId) {
  ...
  if (u.streakLastDate === yesterday) {
    ...
    // Workshop Streak Shield Charm: auto-saves once per week
    const hasStreakCharm = getWorkshopBonus(userId, 'streak_charm') > 0;
    const charmLastUsed = u.streakCharmLastUsed || null;
    const charmCooldownMs = 7 * 24 * 60 * 60 * 1000; // 7 days
    const charmReady = hasStreakCharm && (!charmLastUsed || (Date.now() - new Date(charmLastUsed).getTime()) > charmCooldownMs);
    if (charmReady) {
      u.streakCharmLastUsed = new Date().toISOString();
      u.streakDays = (u.streakDays || 0) + 1;
    } else
