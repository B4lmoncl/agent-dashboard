...
function getTalentEffects(userId) {
  if (!_getUserTalentEffects) {
    try { _getUserTalentEffects = require('../routes/talent-tree').getUserTalentEffects; } catch { /* not loaded yet */ }
  }
  return _getUserTalentEffects ? _getUserTalentEffects(userId) : {};
}
...
function rollCraftingMaterials(questRarity, materialDoubleChance, userProfessions, userId, contentTier) {
  ...
  // Talent: guaranteed_material_drop — additional % chance for material drops
  const talentMatChance = userId ? (getTalentEffects(userId).guaranteed_material_drop || 0) : 0;
  const drops = [];
...
function onQuestCompletedByUser(userId, quest) {
  ...
  let varietyBonus = 1;
  const talentVariety = talentEffects.variety_chain_bonus || 0;
  if (legendaryMods.varietyBonusPct > 0 || talentVariety > 0) {
    ...
    varietyBonus = 1 + (typesToday * legendaryMods.varietyBonusPct) / 100 + (typesToday * talentVariety);
  }
  ...
  let xpWithoutRested = Math.round(xpBase * (xpMulti || 1) * (gearBonus || 1) * (companionBonus || 1) * (bondBonus || 1) * (hoardingMalus || 1) * (passiveXpBonus || 1) * (legendaryMods.xpBonus || 1) * (activeXpBuf || 1) * (nthBonus || 1) * (varietyBonus || 1) * (critMulti || 1) * (armorTraitMod || 1) * dailyDiminishing);
  ...
  if (quest.type === 'companion' && u.companion) {
    ...
    u.companion.bondXp = (u.companion.bondXp || 0) + 1 * charismaBonus * bondLegendaryBonus * talentBondBonus;
    u.companion.bondLevel = getBondLevel(u.companion.bondXp).level;
  ...
  awardUserGold(userId, quest.rarity || quest.priority || 'common', u.streakDays, questGoldBuf * dailyDiminishing);
