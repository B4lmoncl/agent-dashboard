...
const {
  ...
  XP_BY_RARITY, GOLD_BY_RARITY, TEMP_BY_RARITY, RUNENSPLITTER_BY_RARITY,
...
// ─── Economy helpers ────────────────────────────────────────────────────────
function awardUserGold(userId, rarity, streakDays) {
  const u = state.users[userId];
  if (!u) return;
  ...
  let goldEarned = Math.round(base * streakMulti * forgeMulti * legendaryGoldMod);
...
function updateUserForgeTemp(userId, rarity) {
  const u = state.users[userId];
  if (!u) return;
  ...
  const baseRecovery = TEMP_BY_RARITY[rarity] || TEMP_BY_PRIORITY[rarity] || 5;
...
function getWeisheitBonus(userId) {
  const stats = getUserStats(userId);
  return +(1 + (stats.weisheit || 0) * 0.005).toFixed(4);
}
...
function onQuestCompletedByUser(userId, quest) {
  ...
  const xpBase = (XP_BY_RARITY[quest.rarity] || XP_BY_PRIORITY[quest.priority] || 10) + (userStats.fokus || 0); // minor stat: fokus adds flat XP
  ...
  const legendaryMods = getLegendaryModifiers(userId);
  const xpEarned = Math.round(xpBase * xpMulti * gearBonus * companionBonus * bondBonus * hoardingMalus * passiveXpBonus * legendaryMods.xpBonus);
  ...
  awardUserGold(userId, quest.rarity || quest.priority || 'common', u.streakDays);
  updateUserForgeTemp(userId, quest.rarity || quest.priority || 'common');
...
function awardAgentGold(agentKey, priority, streakDays) {
  ...
  const multiplier = Math.min(1 + (streakDays || 0) * 0.015, 1.45);
