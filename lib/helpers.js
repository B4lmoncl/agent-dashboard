...
function rollCraftingMaterials(questRarity, materialDoubleChance) {
...
function onQuestCompletedByUser(userId, quest) {
  ...
  if (legendaryMods.varietyBonusPct > 0) {
    ...
    const typesToday = tc?.types ? Object.keys(tc.types).length : 1;
  ...
  // ─── Crafting material drops ───────────────────────────────────────────────
  const droppedMaterials = rollCraftingMaterials(quest.rarity || 'common', legendaryMods.materialDoubleChance);
