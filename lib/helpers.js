...
function updateUserStreak(userId) {
  ...
  if (u.streakLastDate === yesterday) {
    ...
    const charmUsesThisWeek = (u.streakCharmUses || []).filter(ts => new Date(ts).getTime() >= weekStart.getTime()).length;
...
function awardUserGold(userId, rarity, streakDays, extraMulti) {
  ...
  let goldEarned = Math.round(base * streakMulti * forgeMulti * legendaryGoldMod * leatherGoldMod * (extraMulti || 1));
...
function getLegendaryEffects(userId) {
  ...
  // Include Kanai's Cube active effects
  ...
  if (cube) {
    for (const slot of ['offensive', 'defensive', 'utility']) {
      ...
      if (active?.type && active?.value != null) {
        effects.push({ type: active.type, value: active.value, itemName: `Kanai's Cube (${slot})`, itemId: `cube-${slot}` });
      }
    }
  }
  return effects;
...
function onQuestCompletedByUser(userId, quest) {
  ...
  const xpEarned = Math.round(xpBase * xpMulti * gearBonus * companionBonus * bondBonus * hoardingMalus * passiveXpBonus * legendaryMods.xpBonus * activeXpBuf * nthBonus * varietyBonus * critMulti * armorTraitMod);
  ...
  let dropChance = pityGuaranteed ? 1 : ((hasLuckBuff ? 0.45 : 0.25) + glueckBonus + lootAmuletBonus);
