...
function updateUserForgeTemp(userId, priority) {
  ...
  const baseRecovery = TEMP_BY_PRIORITY[priority] || 5;
  const tempoBonus = (_getRawStats(userId).tempo || 0); // minor stat: tempo adds flat forge temp recovery
  u.forgeTemp = Math.min(100, (u.forgeTemp ?? 100) + baseRecovery + tempoBonus);
...
function getLegendaryModifiers(userId) {
  ...
  return { xpBonus: 1 + xpBonus / 100, goldBonus: 1 + goldBonus / 100, dropBonus, decayReduction: decayReduction / 100, effects };
}
...
function onQuestCompletedByUser(userId, quest) {
  ...
  const xpBase = (XP_BY_PRIORITY[quest.priority] || 10) + (userStats.fokus || 0); // minor stat: fokus adds flat XP
  ...
  awardCurrency(userId, 'runensplitter', quest.priority === 'high' ? 3 : quest.priority === 'medium' ? 2 : 1);
