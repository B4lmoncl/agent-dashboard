...
function onQuestCompletedByUser(userId, quest) {
  ...
  for (const buff of u.activeBuffs) {
    ...
    else if (buff.type === 'ausdauer_boost') { /* decay reduction applied in calcDynamicForgeTemp — reading activeBuffs there */ }
    else if (buff.type === 'fokus_boost') { passiveXpBonus += (buff.amount || buff.value || 2); } // flat XP per quest
    else if (buff.type === 'charisma_boost') { /* bond XP multiplier applied in companion section below */ }
    else if (buff.type === 'tempo_boost') { /* forge recovery applied in updateUserForgeTemp — reading activeBuffs there */ }
    else if (buff.type === 'vitalitaet_boost') { /* streak protection applied in updateUserStreak — reading activeBuffs there */ }
