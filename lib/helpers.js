...
function getQuestHoardingMalus(userId) {
  ...
  // Build quest status lookup once (O(n) → O(1) per check)
  const questStatusById = new Map();
  for (const q of state.quests) questStatusById.set(q.id, q.status);
  const claimedIds = pp.claimedQuests || [];
  const claimedInProgress = claimedIds.filter(qid => {
    const status = questStatusById.get(qid);
...
function getKraftBonus(userId) {
  ...
  return +(1 + (stats.kraft || 0) * 0.01).toFixed(4);
}
...
function getWeisheitBonus(userId) {
  const stats = getUserStats(userId);
  return +(1 + (stats.weisheit || 0) * 0.01).toFixed(4);
}
...
function onQuestCompletedByUser(userId, quest) {
  ...
  const xpEarned = Math.round(xpBase * xpMulti * gearBonus * companionBonus * bondBonus * hoardingMalus * passiveXpBonus);
