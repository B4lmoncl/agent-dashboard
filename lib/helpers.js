...
function paginate(array, query, defaultLimit = 100) {
  ...
  const limit = Math.min(Math.max(parseInt(query.limit) || defaultLimit, 1), 500);
...
function updateUserForgeTemp(userId) {
  ...
  // Minor stat: tempo adds percentage-based forge temp recovery (2% per point)
  const tempoMulti = 1 + tempo * 0.02;
...
function rollAffixStats(template) {
  ...
  if (primary && primary.pool && primary.pool.length > 0) {
    ...
    const shuffled = [...primary.pool].sort(() => Math.random() - 0.5);
  ...
  if (minor && minor.pool && minor.pool.length > 0) {
    ...
    const count = minCount + Math.floor(Math.random() * (maxCount - minCount + 1));
    if (count > 0) {
      const shuffled = [...minor.pool].sort(() => Math.random() - 0.5);
      const picked = shuffled.slice(0, Math.min(count, shuffled.length));
      for (const affix of picked) {
        stats[affix.stat] = affix.min + Math.floor(Math.random() * (affix.max - affix.min + 1));
      }
...
function getLegendaryModifiers(userId) {
  ...
  return { xpBonus: 1 + xpBonus / 100, goldBonus: 1 + goldBonus / 100, dropBonus, decayReduction: decayReduction / 100, streakProtection, effects };
}
...
function getAchievementPoints(userId) {
  ...
  for (const a of earned) {
    const template = state.ACHIEVEMENT_CATALOGUE.find(t => t.id === a.id);
...
function checkAndAwardAchievements(userId) {
  ...
  if (newOnes.length > 0) {
    ...
    if (milestoneUnlocks.length > 0) {
      u._newMilestoneUnlocks = milestoneUnlocks;
    }
  }
  return newOnes;
}
...
function rollCraftingMaterials(questRarity) {
...
function onQuestCompletedByUser(userId, quest) {
  ...
  const consumedBuffs = [];
  for (let i = u.activeBuffs.length - 1; i >= 0; i--) {
    const buff = u.activeBuffs[i];
    ...
    else if (buff.type === 'luck_boost_20') { /* luck handled in drop chance below */ }
    else if (buff.type === 'double_reward') { activeXpBuf *= 2.0; activeGoldBuf *= 2.0; }
    buff.questsRemaining--;
    if (buff.questsRemaining <= 0) consumedBuffs.push(i);
  }
  for (const idx of consumedBuffs) u.activeBuffs.splice(idx, 1);
  const xpEarned = Math.round(xpBase * xpMulti * gearBonus * companionBonus * bondBonus * hoardingMalus * passiveXpBonus * legendaryMods.xpBonus * activeXpBuf);
  ...
  awardCurrency(userId, 'runensplitter', RUNENSPLITTER_BY_RARITY[quest.rarity] || 1);
  ...
  // Check for luck buff from alchemist potions
  const hasLuckBuff = u.activeBuffs?.some(b => b.type === 'luck_boost_20' && b.questsRemaining > 0);
  ...
  // ─── Crafting material drops ───────────────────────────────────────────────
  const droppedMaterials = rollCraftingMaterials(quest.rarity || 'common');
  if (droppedMaterials.length > 0) {
    ...
    u._lastMaterials = droppedMaterials;
  }
  saveUsers();
  return newAchs;
}
