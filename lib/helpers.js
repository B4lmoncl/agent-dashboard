...
function getKraftBonus(userId) {
  ...
  // Soft cap at 20 kraft: 50% of max 30% bonus. At 20: +15%, at 60: +22.5%. Never hard caps.
  return +(1 + diminishing(stats.kraft || 0, 0.30, 20)).toFixed(4);
}
...
function getWeisheitBonus(userId) {
  const stats = getUserStats(userId);
  // Soft cap at 20 weisheit: 50% of max 25% gold bonus. At 20: +12.5%, at 60: +18.75%.
  return +(1 + diminishing(stats.weisheit || 0, 0.25, 20)).toFixed(4);
}
