...
function awardUserGold(userId, priority, streakDays) {
  ...
  const goldEarned = Math.round(base * streakMulti * forgeMulti);
...
function getQuestHoardingMalus(userId) {
  ...
  const activeNpcIds = new Set((state.npcState?.activeNpcs || []).map(npc => npc.id));
  const npcInProgress = Object.entries(pp.npcQuests || {})
    .filter(([npcId, nq]) => {
      // Only count if: (1) status is in_progress, (2) NPC is in active rotation, (3) player has actually started (completedQuests > 0)
      return nq.status === 'in_progress' && activeNpcIds.has(npcId) && (nq.completedQuests || 0) > 0;
    }).length;
...
function calcDynamicForgeTemp(userId) {
  ...
  // Decay at exactly 2% per hour since last update
  const hoursElapsed = (Date.now() - new Date(u.forgeTempAt).getTime()) / 3600000;
  const decayed = stored - hoursElapsed * 2;
...
function onQuestCompletedByUser(userId, quest) {
  ...
  u.xp = (u.xp || 0) + Math.round(xpBase * xpMulti * gearBonus * companionBonus * bondBonus * hoardingMalus);
