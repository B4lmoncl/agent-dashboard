...
function createUniqueInstance(uniqueTemplate) {
  // Roll stats from affixes (same system as normal gear)
  const { stats, legendaryEffect } = rollAffixStats(uniqueTemplate);
  return {
    instanceId: `gi-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
...
function rollCraftingMaterials(questRarity, materialDoubleChance, userProfessions, userId) {
  ...
  for (const [matId, chance] of Object.entries(rateTable)) {
    ...
    if (Math.random() < (chance + magnetBonus)) {
...
function onQuestCompletedByUser(userId, quest) {
  ...
  // ─── Rested XP (WoW Classic style) ────────────────────────────────────
  // If player hasn't completed a quest in 24h+, grant Rested buff
  ...
  if (dailyCount === 1 && hoursSinceLastQuest >= 24 && !u.activeBuffs?.some((b) => b.type === 'rested_xp' && (b.questsRemaining ?? 0) > 0)) {
    u.activeBuffs = u.activeBuffs || [];
    // Rested bonus scales with time away: 1 quest per 2h offline, max 30
    // 24h away = 12 quests, 48h = 24, 60h+ = 30 (cap)
    const restedQuests = Math.min(30, Math.floor(hoursSinceLastQuest / 2));
    u.activeBuffs.push({ type: 'rested_xp', questsRemaining: restedQuests, activatedAt: now(), label: `Rested — +50% XP for ${restedQuests} quests` });
  }
  u._lastQuestCompletedAt = now();

  // Rested XP multiplier (consumed per quest in the buff loop below)
  let restedMulti = 1.0;
  const restedBuff = (u.activeBuffs || []).find((b) => b.type === 'rested_xp' && (b.questsRemaining ?? 0) > 0);
  if (restedBuff) {
    restedMulti = 1.5;
    restedBuff.questsRemaining = (restedBuff.questsRemaining || 1) - 1;
    if (restedBuff.questsRemaining <= 0) {
      u.activeBuffs = (u.activeBuffs || []).filter((b) => b.type !== 'rested_xp' || b !== restedBuff);
    }
  }
  ...
  for (const buff of u.activeBuffs) {
    ...
    if (buff.chargesRemaining) buff.chargesRemaining--;
  }
  ...
  let xpEarned = Math.round(xpBase * (xpMulti || 1) * (gearBonus || 1) * (companionBonus || 1) * (bondBonus || 1) * (hoardingMalus || 1) * (passiveXpBonus || 1) * (legendaryMods.xpBonus || 1) * (activeXpBuf || 1) * (nthBonus || 1) * (varietyBonus || 1) * (critMulti || 1) * (armorTraitMod || 1) * dailyDiminishing * restedMulti);
  if (!isFinite(xpEarned) || isNaN(xpEarned)) xpEarned = xpBase; // Fallback to base XP on any NaN/Infinity
  ...
  if (quest.type === 'companion' && u.companion) {
    ...
    u.companion.bondXp = (u.companion.bondXp || 0) + 1 * charismaBonus * bondLegendaryBonus;
    u.companion.bondLevel = getBondLevel(u.companion.bondXp).level;
  ...
  awardUserGold(userId, quest.rarity || quest.priority || 'common', u.streakDays, activeGoldBuf * dailyDiminishing);
