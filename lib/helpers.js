...
function onQuestCompletedByUser(userId, quest) {
  ...
  if (quest.type === 'companion' && u.companion) {
    ...
    // Charisma from gear + active charisma_boost buff (saved before decrement loop to avoid off-by-one)
    ...
    const bondLegendaryBonus = 1 + (legendaryMods.companionBondBoost || 0); // legendary: companion_bond_boost (from gear)
    const bondBuffBonus = 1 + savedCompBondBoostPct; // active buff: companion_bond_boost (from consumable)
    const talentBondBonus = 1 + (talentEffects.companion_bond_xp_bonus || 0); // talent: companion bond XP
    const actualBondXp = +(1 * charismaBonus * bondLegendaryBonus * bondBuffBonus * talentBondBonus).toFixed(2);
