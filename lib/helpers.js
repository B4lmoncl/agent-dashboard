...
const _passiveEffectCache = new Map();
...
function getLevelInfo(xp) {
  ...
  // Guard against a data error where two levels share the same xpRequired —
  // denominator would be 0 and progress would propagate NaN to the frontend.
  const span = next ? (next.xpRequired - current.xpRequired) : 0;
  const progress = next && span > 0
    ? Math.min(1, (xp - current.xpRequired) / span)
    : 1;
  return {
    level: current.level,
    title: current.title,
...
function updateUserStreak(userId) {
  ...
  if (!u) return;
  // Skip streak update if player is in tavern rest mode (but auto-expire if duration elapsed)
  if (u.tavernRest?.active) {
    const restEnd = new Date(u.tavernRest.startedAt).getTime() + (u.tavernRest.days || 1) * 86400000;
    if (Date.now() <= restEnd) return;
    // Duration elapsed — auto-expire rest and continue with normal streak logic
  ...
  if (u.streakLastDate === yesterday) {
    ...
    // Affix: necrotic — all streak protection disabled during active rift
    const necroticBlocked = !!u._riftNecroticActive;
    // Workshop Streak Shield Charm: auto-saves N times per week (tier 1=1x, tier 2=2x)
    const charmMaxUses = necroticBlocked ? 0 : getWorkshopBonus(userId, 'streak_charm');
    ...
    if (!necroticBlocked && (u.streakShields || 0) > 0) {
      u.streakShields -= 1;
      u.streakDays = (u.streakDays || 0) + 1;
    } else {
      // Minor stat: vitalitaet adds +1% streak protection chance per point (blocked by necrotic)
      const vitBonus = necroticBlocked ? 0 : (_getRawStats(userId).vitalitaet || 0) * 0.01;
      const passiveRecovery = necroticBlocked ? 0 : (hasPassiveEffect(userId, 'streak_recovery_50') ? 0.5 : 0);
      const totalRecoveryChance = Math.min(0.75, passiveRecovery + vitBonus); // cap at 75%
      if (totalRecoveryChance > 0 && Math.random() < totalRecoveryChance) {
        u.streakDays = (u.streakDays || 0) + 1;
      } else {
        // Talent: streak_break_buffer — retain % of streak bonus for N days
        const breakBuffer = getTalentEffects(userId).streak_break_buffer;
        if (breakBuffer && u.streakDays > 1) {
          u._streakBuffer = {
            retainedDays: Math.round((u.streakDays || 0) * (breakBuffer.retainPercent || 0.5)),
            expiresAt: new Date(Date.now() + (breakBuffer.durationDays || 3) * 86400000).toISOString(),
          };
        }
        u.streakDays = 1;
      }
  ...
  for (const m of STREAK_MILESTONES) {
    if (streakDays === m.days) {
      ...
      u._lastStreakMilestone = { days: m.days, label: m.label, badge: m.badge };
...
function awardUserGold(userId, rarity, streakDays, extraMulti) {
  ...
  // Talent: streak_break_buffer — use retained streak days if buffer is active
  let effectiveStreak = streakDays || 0;
  if (u._streakBuffer && effectiveStreak <= 1) {
    const bufferExpired = new Date(u._streakBuffer.expiresAt).getTime() < Date.now();
    if (!bufferExpired) {
      effectiveStreak = Math.max(effectiveStreak, u._streakBuffer.retainedDays || 0);
    } else {
      delete u._streakBuffer; // expired, clean up
    }
  }
  // Streak gold bonus: +0.5% per day, soft cap via diminishing returns (max ~20% at 60+ days)
  const streakMulti = 1 + diminishing(effectiveStreak, 0.20, 30);
  ...
  let goldEarned = Math.floor(base * (streakMulti || 1) * (forgeMulti || 1) * (legendaryGoldMod || 1) * (leatherGoldMod || 1) * (extraMulti || 1));
  ...
  // u.currencies.gold is source of truth (established in shop.js wave 73).
  // Reading u.gold first here is a drift risk on the hot path: every quest
  // completion used to flow through this. If any other endpoint wrote
  // u.currencies.gold without syncing u.gold, this overwrote the new value.
  ensureUserCurrencies(u);
  u.currencies.gold = (u.currencies.gold ?? u.gold ?? 0) + goldEarned;
...
function getXpMultiplier(userId) {
  // D3-style: forge + kraft are additive within the Forge bucket
  return +(1 + (getForgeXpBase(userId) - 1) + (getKraftBonus(userId) - 1)).toFixed(4);
}
...
function getGoldMultiplier(userId) {
  ...
  // D3-style: forge + weisheit + workshop are additive within the Forge bucket
  return +(1 + (getForgeGoldBase(userId) - 1) + (getWeisheitBonus(userId) - 1) + workshopGoldPct / 100).toFixed(4);
}
...
function _buildPassiveEffectSet(userId) {
  if (_passiveEffectCache.has(userId)) return _passiveEffectCache.get(userId);
  const u = state.users[userId];
  const effects = new Set();
  if (u && u.inventory) {
    const { resolveItem } = require('./state');
    for (const inv of u.inventory) {
      const tmpl = resolveItem(inv.itemId || inv.id);
      if (!tmpl || tmpl.type !== 'passive') continue;
      const eff = tmpl.effect;
      if (typeof eff === 'string') effects.add(eff);
      else if (eff && eff.type) effects.add(eff.type);
    }
  }
  _passiveEffectCache.set(userId, effects);
  return effects;
}

function hasPassiveEffect(userId, effectString) {
  return _buildPassiveEffectSet(userId).has(effectString);
}
...
function addLootToInventory(userId, lootItem) {
  ...
  // Auto-consume effects directly without push/filter cycle
  const effectType = lootItem.effect?.type;
  if (effectType === 'gold') {
    ensureUserCurrencies(u); u.currencies.gold = (u.currencies.gold ?? u.gold ?? 0) + lootItem.effect.amount; u.gold = u.currencies.gold;
    return;
  }
  if (effectType === 'xp') {
    grantPlayerXp(u, lootItem.effect.amount);
    return;
  }
...
function checkAndAwardAchievements(userId) {
  ...
  if (newOnes.length > 0) {
    const milestoneUnlocks = checkAchievementMilestones(userId);
    if (milestoneUnlocks.length > 0) {
      u._lastMilestoneUnlocks = milestoneUnlocks;
    }
  }
  return newOnes;
}
...
function rollCraftingMaterials(questRarity, materialDoubleChance, userProfessions, userId, contentTier) {
  ...
  const drops = [];
  if (!state._materialById) state._materialById = new Map((state.professionsData?.materials || []).map(m => [m.id, m]));
  const matLookup = state._materialById;
  ...
  for (const [matId, chance] of Object.entries(rateTable)) {
    ...
    if (Math.random() < (chance + magnetBonus + talentMatChance)) {
      ...
      const matDef = matLookup.get(matId);
      drops.push({ id: matId, name: matDef?.name || matId, amount });
    }
  }
    ...
    for (const profDef of state.professionsData.professions) {
      ...
      for (const matId of profDef.gatheringAffinity) {
        if (Math.random() < bonusChance) {
          ...
          else drops.push({ id: matId, name: matLookup.get(matId)?.name || matId, amount: 1, gathering: true });
        }
      }
    }
  }
...
function onQuestCompletedByUser(userId, quest) {
  ...
  // Diminishing: smooth curve instead of hard cliffs
  // 1-5 = 100%, 6-7 = 90%, 8-10 = 75%, 11-15 = 60%, 16-20 = 50%, 21+ = 25%
  const dailyDiminishing = dailyCount <= 5 ? 1.0 : dailyCount <= 7 ? 0.90 : dailyCount <= 10 ? 0.75 : dailyCount <= 15 ? 0.60 : dailyCount <= 20 ? 0.50 : 0.25;
  ...
  // Capture companion-related buff values BEFORE the decrement loop removes them (off-by-one fix)
  const _savedCharismaBoost = u.activeBuffs.find(b => b.type === 'charisma_boost' && ((b.questsRemaining || 0) > 0 || (b.chargesRemaining || 0) > 0));
  const savedCharismaAmount = _savedCharismaBoost ? (_savedCharismaBoost.amount || _savedCharismaBoost.value || 3) : 0;
  const _savedCompBondBoost = u.activeBuffs.find(b => b.type === 'companion_bond_boost' && ((b.questsRemaining || 0) > 0 || (b.chargesRemaining || 0) > 0));
  const savedCompBondBoostPct = _savedCompBondBoost ? (_savedCompBondBoost.amount || _savedCompBondBoost.value || 0.1) : 0;
  for (const buff of u.activeBuffs) {
    ...
    // D3-style: all buffs within the Buff bucket are ADDITIVE with each other
    if (buff.type === 'xp_boost_10') activeXpBuf += 0.10;
    else if (buff.type === 'xp_boost_5') activeXpBuf += 0.05;
    else if (buff.type === 'xp_boost_15') activeXpBuf += 0.15;
    else if (buff.type === 'xp_boost_20') activeXpBuf += 0.20;
    else if (buff.type === 'xp_boost_25_return') activeXpBuf += 0.25;
    else if (buff.type === 'xp_boost_50_perfect') activeXpBuf += 0.50;
    else if (buff.type === 'gold_boost_15') activeGoldBuf += 0.15;
    else if (buff.type === 'gold_boost_10') activeGoldBuf += 0.10;
    else if (buff.type === 'feast_buff') { activeXpBuf += 0.15; activeGoldBuf += 0.10; }
    else if (buff.type === 'luck_boost_20') { hasLuckBuff = true; }
    else if (buff.type === 'luck_boost_10') { hasLuckBuff = true; }
    else if (buff.type === 'double_reward') { activeXpBuf += 1.0; activeGoldBuf += 1.0; }
    else if (buff.type === 'xp_gold_boost') { activeXpBuf += (buff.xpPercent || 15) / 100; activeGoldBuf += (buff.goldPercent || 10) / 100; }
    else if (buff.type === 'luck_boost') { hasLuckBuff = true; }
    // Stat boost buffs from crafting (alchemist potions, cook meals)
    else if (buff.type === 'gold_boost_20') activeGoldBuf += 0.20;
    else if (buff.type === 'gold_boost_5') activeGoldBuf += 0.05;
    else if (buff.type === 'kraft_boost') activeXpBuf += (buff.amount || 2) * 0.005;
    else if (buff.type === 'weisheit_boost') activeGoldBuf += (buff.amount || 2) * 0.004;
    ...
    else if (buff.type === 'fokus_boost') { passiveXpBonus += (buff.amount || buff.value || 2) * 0.01; } // +1% XP per fokus point (e.g. amount=2 → +2%)
    ...
    // Decrement quest-based counters (utility buffs already skipped via continue above)
    if (buff.questsRemaining) buff.questsRemaining--;
    else if (buff.chargesRemaining) buff.chargesRemaining--;
  }
  ...
  if (legendaryMods.varietyBonusPct > 0 || talentVarietyPct > 0) {
    ...
    if (varietyBonus > 1) u._lastVarietyBonus = { types: typesToday, bonus: Math.round((varietyBonus - 1) * 100) };
  }
  // Legendary + Talent: night bonuses. Use Europe/Berlin explicitly — a UTC
  // server would otherwise treat 21:00 Berlin as 19:00 local and never fire.
  const berlinParts = new Intl.DateTimeFormat('en-GB', {
    timeZone: TIMEZONE, hour: '2-digit', weekday: 'short', hour12: false,
  }).formatToParts(new Date());
  const hour = parseInt(berlinParts.find(p => p.type === 'hour')?.value || '0', 10);
  const weekday = berlinParts.find(p => p.type === 'weekday')?.value || '';
  ...
  if (isNight && legendaryMods.nightDoubleGold > 0) {
    activeGoldBuf += legendaryMods.nightDoubleGold / 100;
  }
  // Talent: night_xp_bonus — bonus XP during night hours (additive in buff bucket)
  const talentNightXp = talentEffects.night_xp_bonus || 0;
  if (isNight && talentNightXp > 0) {
    activeXpBuf += talentNightXp;
  }
  // Talent: weekend_xp_bonus — bonus XP on weekends (additive in buff bucket)
  const isWeekend = weekday === 'Sat' || weekday === 'Sun';
  const talentWeekendXp = talentEffects.weekend_xp_bonus || 0;
  if (isWeekend && talentWeekendXp > 0) {
    activeXpBuf += talentWeekendXp;
  }
  ...
  if (legendaryMods.berserkerBonus > 0 && calcDynamicForgeTemp(userId) > 80) {
    activeXpBuf += legendaryMods.berserkerBonus;
  }
  ...
  // D3-style bucket system: additive within bucket, multiplicative between buckets
  // Gear bucket: gear tier + armor trait + codex (additive)
  const armorTraitXpBonus = getArmorTraitBonus(userId).xpBonus;
  const gearBucket = 1 + ((gear.xpBonus || 0) / 100) + armorTraitXpBonus + (codexXpBonus - 1);
  // Companion bucket: companion achievements + bond level (additive)
  const companionBucket = 1 + (companionBonus - 1) + (bondBonus - 1);
  // Equipment effects bucket: passive items + legendary xpBonus (additive)
  const equipBucket = 1 + (passiveXpBonus - 1) + ((legendaryMods.xpBonus || 1) - 1);
  // Situational bucket: variety + chain + nth (additive)
  const situationalBucket = 1 + (varietyBonus - 1) + (chainBonus - 1) + (nthBonus - 1);
  // Calculate base XP (without rested). Buckets are multiplicative with each other.
  let xpWithoutRested = Math.round(xpBase * (xpMulti || 1) * gearBucket * companionBucket * (hoardingMalus || 1) * equipBucket * (activeXpBuf || 1) * situationalBucket * (critMulti || 1) * (gambleMulti || 1) * dailyDiminishing);
  ...
  if (quest.type === 'companion' && u.companion) {
    ...
    // Bond XP bonuses are all the same category (extra bond XP from various
    // sources) — per D3 bucket rule they stack additively, not multiplicatively.
    // Old: 1.5 × 1.2 × 1.1 × 1.2 ≈ 2.38x (exponential). New: 1 + 0.5 + 0.2 +
    // 0.1 + 0.2 = 2.0x (flat). Prevents runaway endgame bond XP.
    const totalCharisma = (userStats.charisma || 0) + savedCharismaAmount;
    const charismaPct = totalCharisma * 0.05; // minor stat: +5% bond XP per charisma
    const bondLegendaryPct = legendaryMods.companionBondBoost || 0; // legendary: companion_bond_boost (from gear)
    const bondBuffPct = savedCompBondBoostPct; // active buff: companion_bond_boost (from consumable)
    const talentBondPct = talentEffects.companion_bond_xp_bonus || 0; // talent: companion bond XP
    const bondBucket = 1 + charismaPct + bondLegendaryPct + bondBuffPct + talentBondPct;
    const actualBondXp = +(bondBucket).toFixed(2);
  ...
  // Track time-based completion counters for achievements. Must use
  // Europe/Berlin — a UTC server would undercount night/weekend completions.
  const compParts = new Intl.DateTimeFormat('en-GB', {
    timeZone: TIMEZONE, hour: '2-digit', weekday: 'short', hour12: false,
  }).formatToParts(new Date());
  const completionHour = parseInt(compParts.find(p => p.type === 'hour')?.value || '0', 10);
  if (completionHour >= 22 || completionHour < 5) {
    u._nightCompletions = (u._nightCompletions || 0) + 1;
  }
  const compWeekday = compParts.find(p => p.type === 'weekday')?.value || '';
  if (compWeekday === 'Sat' || compWeekday === 'Sun') {
  ...
  } catch (e) { if (e.code !== 'MODULE_NOT_FOUND') console.warn('[factions] grantReputation error:', e.message); }
  // ── Battle Pass XP ──
  try {
  ...
  } catch (e) { if (e.code !== 'MODULE_NOT_FOUND') console.warn('[battlepass] grantBattlePassXP error:', e.message); }
  // ── World Boss damage ──
  try {
  ...
  if (droppedLoot) {
    ...
    // Only surface the loot to the celebration popup if the item actually
    // landed in the inventory. When the bag was full, addLootToInventory
    // sets u._inventoryFull and drops the item on the floor — showing it
    // in the reward popup next to a "loot lost" toast is confusing.
    if (!u._inventoryFull) {
      // Enrich loot with richer template data (desc, flavorText, effect) so
      // the frontend hover tooltip can display the full item card.
      const tmplId = droppedLoot.templateId || droppedLoot.id;
      const tmpl = tmplId && state.itemTemplates ? state.itemTemplates.get(tmplId) : null;
      u._lastLoot = tmpl ? { ...tmpl, ...droppedLoot, desc: droppedLoot.desc || tmpl.description || null, flavorText: droppedLoot.flavorText || tmpl.flavorText || null } : droppedLoot;
    }
  }
  ...
  if (droppedMaterials.length > 0) {
    ...
    u._lastMaterialDrops = droppedMaterials;
  }
  // ─── Gem drops ──────────────────────────────────────────────────────────────
  const gemsData = state.gemsData || require('../public/data/gems.json');
  ...
  if (echoEffect && xpEarned > 0) {
    ...
    if (echoXp > 0) {
      // Resolve friends from socialData.friendships (not u.friends which doesn't exist)
      const friendships = state.socialData?.friendships || [];
      const friendIds = friendships
        .filter(f => f.status === 'accepted' && (f.player1 === userId || f.player2 === userId))
        .map(f => f.player1 === userId ? f.player2 : f.player1);
      for (const friendId of friendIds) {
        const friend = state.users[friendId];
        if (!friend) continue;
        grantPlayerXp(friend, echoXp);
        friend._lastEchoXp = { from: userId, amount: echoXp };
      }
    }
  }

  // ─── Sworn Bond contribution ──────────────────────────────────────────────
  try { const { contributeToBond } = require('../routes/sworn-bonds'); contributeToBond(userId, quest); } catch (e) { if (e.code !== 'MODULE_NOT_FOUND') console.warn('[sworn-bond] contribution error:', e.message); }
...
// ─── Shared XP helper — level-up cascade ────────────────────────────────────
// Every site that grants XP to a player (outside the quest-completion hot path
// in addQuestRewards, which has its own full cascade) should use this. Handles:
//   - u.xp + u.seasonXp increment
//   - Level crossing detection
//   - Stardust reward on level-up (5 + newLevel)
//   - Returns { xpEarned, prevLevel, newLevel, leveledUp, title }
// Callers can surface the leveledUp flag in their response so the frontend
// can fire a celebration. Before this helper, 10 XP sites (campaigns,
// expedition, ritual, pact, habits, loot, friend echo) added XP raw and
// silently skipped level-up celebrations.
function grantPlayerXp(userId, amount, opts = {}) {
  const u = typeof userId === 'object' ? userId : state.users[(userId || '').toLowerCase()];
  if (!u) return { xpEarned: 0, leveledUp: false };
  const xp = Math.max(0, Math.floor(Number(amount) || 0));
  if (xp === 0) return { xpEarned: 0, leveledUp: false };
  const prevLevel = getLevelInfo(u.xp || 0).level;
  u.xp = (u.xp || 0) + xp;
  if (opts.countSeasonXp !== false) u.seasonXp = (u.seasonXp || 0) + xp;
  const levelInfo = getLevelInfo(u.xp);
  const newLevel = levelInfo.level;
  const leveledUp = newLevel > prevLevel;
  if (leveledUp) {
    // Stardust reward mirrors the quest-completion cascade formula
    // (lib/helpers.js:1884 in addQuestRewards).
    const uid = u.id || (typeof userId === 'string' ? userId.toLowerCase() : null);
    if (uid) awardCurrency(uid, 'stardust', 5 + newLevel);
  }
  return { xpEarned: xp, prevLevel, newLevel, leveledUp, title: levelInfo.title };
}
