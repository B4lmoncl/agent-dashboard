...
function updateUserStreak(userId) {
  ...
  // Streak milestone titles — earned once, kept forever (Duolingo-style streak cosmetics)
  const STREAK_TITLES = [
    { days: 7,   titleId: 'streak-bronze',   name: 'Flammenhüter',       rarity: 'uncommon' },
    { days: 30,  titleId: 'streak-silver',   name: 'Unerschütterlich',   rarity: 'rare' },
    { days: 90,  titleId: 'streak-gold',     name: 'Eiserner Wille',     rarity: 'epic' },
    { days: 180, titleId: 'streak-diamond',  name: 'Diamantflamme',      rarity: 'epic' },
    { days: 365, titleId: 'streak-legend',   name: 'Die Ewige Flamme',   rarity: 'legendary' },
  ];
  for (const m of STREAK_MILESTONES) {
    if (streakDays === m.days) {
      logActivity(userId, 'streak_milestone', { days: m.days, badge: m.badge, label: m.label });
      // Award streak title if applicable
      const titleDef = STREAK_TITLES.find(t => t.days === m.days);
      if (titleDef) {
        if (!u.earnedTitles) u.earnedTitles = [];
        if (!u.earnedTitles.some(t => t.id === titleDef.titleId)) {
          u.earnedTitles.push({ id: titleDef.titleId, name: titleDef.name, rarity: titleDef.rarity, source: `${m.days}-day streak`, earnedAt: new Date().toISOString() });
        }
      }
      break;
    }
...
function onQuestCompletedByUser(userId, quest) {
  ...
  for (const buff of u.activeBuffs) {
    ...
    else if (buff.type === 'ausdauer_boost') { /* decay reduction — applied in calcDynamicForgeTemp */ }
    else if (buff.type === 'fokus_boost') { /* flat XP — not multiplicative, handled via passiveXpBonus */ }
    else if (buff.type === 'charisma_boost') { /* bond XP — applied in companion section */ }
    else if (buff.type === 'tempo_boost') { /* forge recovery — applied in updateUserForgeTemp */ }
    else if (buff.type === 'vitalitaet_boost') { /* streak protection — evaluated at streak check */ }
    else if (buff.type === 'craft_xp_boost') { /* craft XP — applied in crafting.js */ }
    else if (buff.type === 'companion_bond_boost') { /* companion bond — applied in bond section */ }
  ...
  awardUserGold(userId, quest.rarity || 'common', u.streakDays, questGoldBuf * dailyDiminishing);
