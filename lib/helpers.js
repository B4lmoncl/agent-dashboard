...
function getKraftBonus(userId) {
  ...
  // Soft cap at 20 kraft: 50% of max 20% bonus. At 60 kraft: ~15%. Never hard caps.
  return +(1 + diminishing(stats.kraft || 0, 0.20, 20)).toFixed(4);
}
...
function getWeisheitBonus(userId) {
  const stats = getUserStats(userId);
  // Soft cap at 20 weisheit: 50% of max 15% gold bonus. At 60: ~11.25%. Never hard caps.
  return +(1 + diminishing(stats.weisheit || 0, 0.15, 20)).toFixed(4);
}
...
function createGearInstance(template) {
  ...
  if (template.fixedStats) {
    ...
    ({ stats, legendaryEffect } = rollAffixStats(template));
  }
...
function createNpcGearInstance(npcItem) {
  ...
  const gemsData = state.gemsData || require('../public/data/gems.json');
  const npcSocketRange = gemsData.socketsByRarity[npcItem.rarity || 'rare'] || [0, 0];
  const npcSocketCount = npcSocketRange[0] + Math.floor(Math.random() * (npcSocketRange[1] - npcSocketRange[0] + 1));
  return {
    instanceId: `gi-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    ...
    stats: npcItem.stats || {},
    sockets: Array(npcSocketCount).fill(null),
    legendaryEffect: null,
...
function getUserDropBonus(userId) {
  ...
  // Diminishing returns: soft cap at 20 glück, max +12% bonus. At 60 glück: ~9%.
  return diminishing(stats.glueck || 0, 0.12, 20) + (legendaryMods.dropBonus || 0);
}
...
function onQuestCompletedByUser(userId, quest) {
  ...
  let xpEarned = Math.round(xpBase * (xpMulti || 1) * (gearBonus || 1) * (companionBonus || 1) * (bondBonus || 1) * (hoardingMalus || 1) * (passiveXpBonus || 1) * (legendaryMods.xpBonus || 1) * (activeXpBuf || 1) * (nthBonus || 1) * (varietyBonus || 1) * (critMulti || 1) * (armorTraitMod || 1));
