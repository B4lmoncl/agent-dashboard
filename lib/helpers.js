...
function getQuestHoardingMalus(userId) {
  ...
  const claimedCount = (pp.claimedQuests || []).length;
  // Also count NPC quests that are in_progress for this player
  const npcInProgress = Object.values(pp.npcQuests || {}).filter(nq => nq.status === 'in_progress').length;
  const total = claimedCount + npcInProgress;
