...
function updateUserStreak(userId) {
  ...
  } else if (u.streakDays > 0 && hasPassiveEffect(userId, 'streak_recovery_50') && Math.random() < 0.5) {
    // Passive: streak_recovery_50 — 50% chance to preserve streak on miss
    // Streak preserved, just continue
    u.streakDays = (u.streakDays || 0) + 1;
  } else {
    u.streakDays = 1;
  }
...
function awardUserGold(userId, priority, streakDays) {
  ...
  const streakMulti = Math.min(1 + (streakDays || 0) * 0.01, 1.3);
  const forgeMulti = getGoldMultiplier(userId);
  let goldEarned = Math.round(base * streakMulti * forgeMulti);
...
function getQuestHoardingMalus(userId) {
  ...
  // pp.claimedQuests is an array of quest IDs, not objects
  // We need to resolve them to actual quest objects and check status
  const claimedIds = pp.claimedQuests || [];
  const claimedInProgress = claimedIds.filter(qid => {
    const quest = state.quests.find(q => q.id === qid);
    return quest && quest.status !== 'done';
  }).length;
  ...
  // Build reverse map: questId -> npcGiverId
  const questIdToNpc = {};
...
function getXpMultiplier(userId) {
  ...
  let base;
  if (temp >= 100) base = 1.5;
  else if (temp >= 80) base = 1.25;
  else if (temp >= 60) base = 1.15;
  else if (temp >= 40) base = 1.0;
  else if (temp >= 20) base = 0.8;
  else base = 0.5;
  // Kraft stat: +1% XP per point
  const stats = getUserStats(userId);
  const kraftBonus = 1 + (stats.kraft || 0) * 0.01;
  return +(base * kraftBonus).toFixed(4);
}
...
function getGoldMultiplier(userId) {
  ...
  let base;
  if (temp >= 100) base = 1.5;
  else if (temp >= 80) base = 1.3;
  else if (temp >= 60) base = 1.15;
  else base = 1.0;
  // Weisheit stat: +1% Gold per point
  const stats = getUserStats(userId);
  const weisheitBonus = 1 + (stats.weisheit || 0) * 0.01;
  return +(base * weisheitBonus).toFixed(4);
}
...
function calcDynamicForgeTemp(userId) {
  ...
  const decayRate = 2 * ausdauerMultiplier;
  const hoursElapsed = (Date.now() - new Date(u.forgeTempAt).getTime()) / 3600000;
  const decayed = stored - hoursElapsed * decayRate;
  return Math.min(100, Math.max(0, Math.round(decayed)));
}

// Raw stats without multiplier effects (avoids circular calls from getUserStats -> calcDynamicForgeTemp -> getUserStats)
function _getRawStats(userId) {
  ...
  for (const itemId of Object.values(equipped)) {
    ...
    if (item) {
      ...
      // Check item templates (gacha/loot equipment)
...
function onQuestCompletedByUser(userId, quest) {
  ...
  u.xp = (u.xp || 0) + Math.round(xpBase * xpMulti * gearBonus * companionBonus * bondBonus * hoardingMalus * passiveXpBonus);
...
function awardAgentGold(agentKey, priority, streakDays) {
  ...
  if (!agent) return;
  const base = randGold(priority);
  const multiplier = Math.min(1 + (streakDays || 0) * 0.01, 1.3);
