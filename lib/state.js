...
function evaluateAchievementCondition(condition, u) {
  ...
  switch (condition.type) {
    ...
    case 'sworn_bond_level': {
      const bond = (state.socialData?.swornBonds || []).find(b => b.status === 'active' && (b.player1 === uid || b.player2 === uid));
      return (bond?.bondLevel || 0) >= count;
    }
    case 'sworn_bond_streak': {
      const sBond = (state.socialData?.swornBonds || []).find(b => b.status === 'active' && (b.player1 === uid || b.player2 === uid));
