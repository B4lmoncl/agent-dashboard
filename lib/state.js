...
const _saveFirstCall = {}; // Track when the first deferred save was scheduled
const MAX_DEFER_MS = 2000; // Force flush after 2s of continuous deferral
function debouncedSave(key, saveFn, delayMs = 200) {
  if (_saveTimers[key]) clearTimeout(_saveTimers[key]);
  _saveFns[key] = saveFn;
  if (!_saveFirstCall[key]) _saveFirstCall[key] = Date.now();
  // If we've been deferring for too long, flush immediately
  const elapsed = Date.now() - _saveFirstCall[key];
  const effectiveDelay = elapsed >= MAX_DEFER_MS ? 0 : delayMs;
  _saveTimers[key] = setTimeout(() => {
    delete _saveTimers[key];
    delete _saveFns[key];
    delete _saveFirstCall[key];
    try {
      saveFn();
    } catch (err) {
...
function evaluateAchievementCondition(condition, u) {
  ...
  switch (condition.type) {
    ...
    case 'boss_defeated':         return (u._bossesDefeated || 0) >= count;
    ...
    case 'codex_discovered':       return (u.codexDiscovered || []).length >= count;
    case 'secret_found':
    case 'easter_egg':            return (u._secretsFound || []).includes(condition.secretId || condition.eggId || true);
    case 'time_of_day': {
      const hour = new Date().getHours();
      if (condition.range === 'night') return hour >= 22 || hour < 5;
      if (condition.range === 'morning') return hour >= 5 && hour < 12;
      if (condition.range === 'afternoon') return hour >= 12 && hour < 18;
      if (condition.range === 'evening') return hour >= 18 && hour < 22;
      return false;
    }
    case 'day_of_week': {
      const day = new Date().getDay();
      if (condition.day === 'weekend') return day === 0 || day === 6;
      if (condition.day === 'weekday') return day >= 1 && day <= 5;
      return day === (condition.dayNum || 0);
    }
    case 'completion_time':
      // Check if player has completed N quests within the given time window
      return (u._nightCompletions || 0) >= (condition.count || 1);
    case 'profession_skill': {
      const maxSkill = Math.max(0, ...Object.values(u.professions || {}).map(p => (p && typeof p === 'object') ? (p.skill || p.xp || 0) : 0));
      return maxSkill >= count;
    }
    case 'crafts_completed':      return (u._craftsCompleted || 0) >= count;
    case 'moonlight_crafts':      return (u._moonlightCrafts || 0) >= count;
    case 'bond_level':            return (u.companion?.bondLevel || 0) >= count;
    case 'sworn_bond_level': {
      const sbUid = u.id || u.name?.toLowerCase();
      const bond = (state.socialData?.swornBonds || []).find(b => b.status === 'active' && (b.player1 === sbUid || b.player2 === sbUid));
      return (bond?.bondLevel || 0) >= count;
    }
    case 'sworn_bond_streak': {
      const ssUid = u.id || u.name?.toLowerCase();
      const sBond = (state.socialData?.swornBonds || []).find(b => b.status === 'active' && (b.player1 === ssUid || b.player2 === ssUid));
      return (sBond?.longestStreak || 0) >= count;
    }
    case 'streak':                return (u.streakDays || 0) >= count;
    case 'level': {
      const { getLevelInfo } = require('./helpers');
      return getLevelInfo(u.xp || 0).level >= count;
    }
    case 'gold':                  return (u.currencies?.gold ?? u.gold ?? 0) >= count;
    case 'rituals_completed':     return (u._ritualsCompleted || 0) >= count;
    case 'friends_count': {
      const friendships = state.socialData?.friendships || [];
      const uid = u.id || u.name?.toLowerCase();
      const fCount = friendships.filter(f => f.player1 === uid || f.player2 === uid).length;
      return fCount >= count;
    }
    case 'trades_completed':      return (u._tradesCompleted || 0) >= count;
    case 'mails_sent':            return (u._mailsSent || 0) >= count;
    case 'faction_standing': {
      const fid = condition.factionId;
      return fid && (u.factions?.[fid]?.rep || 0) >= count;
    }
    case 'mythic_level':          return (u.highestMythicCleared || 0) >= count;
    case 'rift_completions':      return (u._riftCompletions || 0) >= count;
    case 'rift_legendary_completions': return (u.riftHistory || []).filter(h => h.tier === 'legendary' && h.success).length >= count;
    case 'dungeon_completions':   return (u._dungeonCompletions || 0) >= count;
    case 'expedition_completions': return (u._expeditionCompletions || 0) >= count;
    case 'learned_recipes':       return (u.learnedRecipes || []).length >= count;
    case 'battlepass_level':      return (u.battlePass?.level || 0) >= count;
    case 'world_boss_contributions': return (u._worldBossContributions || 0) >= count;
    case 'gacha_hard_pity':       return !!(u._hitHardPity);
    case 'gem_tier': {
      const gems = u.gems || {};
      return Object.keys(gems).some(k => parseInt(k.split('_').pop() || '0', 10) >= count);
    }
    case 'unique_bosses_defeated': {
      const bossKills = new Set((u._bossKillIds || []).map(String));
      return bossKills.size >= count;
    }
    case 'unique_npcs_completed':  return (u._npcsUnlocked || 0) >= count;
    case 'dual_profession_skill': {
      const skills = Object.values(u.professions || {}).map(p => (p && typeof p === 'object') ? (p.skill || 0) : 0).sort((a, b) => b - a);
      return skills.length >= 2 && skills[1] >= count;
    }
    case 'all_professions_skill': {
      const allSkills = Object.values(u.professions || {}).map(p => (p && typeof p === 'object') ? (p.skill || 0) : 0);
      return allSkills.length >= count && allSkills.every(s => s >= (condition.minSkill || 50));
    }
    case 'quest_type_variety':    return (u._allCompletedTypes || []).length >= count;
    case 'all_factions_standing': {
      const factions = u.factions || {};
      const standingNames = Object.values(factions).map(f => {
        const rep = (f && typeof f === 'object') ? (f.rep || 0) : 0;
        return rep >= 15000 ? 'paragon' : rep >= 8000 ? 'exalted' : rep >= 4000 ? 'revered' : rep >= 1500 ? 'honored' : rep >= 500 ? 'friendly' : 'neutral';
      });
      const target = condition.standing || 'friendly';
      const ORDER = ['neutral', 'friendly', 'honored', 'revered', 'exalted', 'paragon'];
      const targetIdx = ORDER.indexOf(target);
      return standingNames.filter(s => ORDER.indexOf(s) >= targetIdx).length >= count;
    }
    case 'faction_dailies_complete': return (u._factionDailiesComplete || 0) >= count;
    case 'dungeon_tiers_complete': {
      const tiers = new Set((u._dungeonTiersCleared || []).map(String));
      return tiers.size >= count;
    }
    case 'world_boss_top3':       return (u._worldBossTop3 || 0) >= count;
    case 'currency_conversions':  return (u._currencyConversions || 0) >= count;
    case 'all_royal_gems': {
      const gems = u.gems || {};
      const royalCount = Object.keys(gems).filter(k => k.endsWith('_5') && (gems[k] || 0) > 0).length;
      return royalCount >= count;
    }
    case 'perfect_weeks':         return (u._perfectWeeks || 0) >= count;
    case 'quest_type_min_each': {
      const typeCounts = { development: u._devCount || 0, learning: u._learningCount || 0, fitness: u._fitnessCount || 0, social: u._socialCount || 0, personal: u._personalCount || 0 };
      return Object.values(typeCounts).every(c => c >= count);
    }
    // Talent tree achievements
    case 'talent_points_spent': {
      const talents = u.talents || {};
      return (talents.totalSpent || 0) >= count;
    }
    case 'talent_respec':           return (u._talentRespecs || 0) >= count;
    // Adventure Tome achievements
    case 'tome_milestone_claimed':  return (u._tomeMilestonesClaimed || 0) >= count;
    case 'tome_total_percent': {
      const claimed = Object.keys(u._tomeClaimed || {}).length;
      return claimed >= count; // count = number of milestones needed
    }
    case 'tome_floor_complete': {
      const floors100 = Object.values(u._tomeFloorPcts || {}).filter(p => p >= 100).length;
      return floors100 >= count;
    }
    // Enchanting / Kanai achievements
    case 'enchantments_done':       return (u._enchantmentsDone || 0) >= count;
    case 'kanai_extractions':       return (u._kanaiExtractions || 0) >= count;
    case 'kanai_slots_filled': {
      const cube = u.kanaisCube || {};
      const filled = ['offensive', 'defensive', 'utility'].filter(s => cube[s]?.type).length;
      return filled >= count;
    }
    // Expedition achievements
    case 'expedition_tier_ancient':  return (u._expeditionAncientClears || 0) >= count;
...
const state = {
  ...
  forgeFever: null, // Schmiedefieber state: { profession, startedAt, lastRotation, playerCrafts, playerClaimed }
  ...
  // Sworn Bond levels (player-to-player pact progression)
  SWORN_BOND_LEVELS: [
    { level: 1,  title: 'Bekannte',        minXp: 0,   goldBonus: 0,  essenzBonus: 0 },
    { level: 2,  title: 'Verbundene',      minXp: 20,  goldBonus: 5,  essenzBonus: 1 },
    { level: 3,  title: 'Geschworene',     minXp: 50,  goldBonus: 10, essenzBonus: 2 },
    { level: 4,  title: 'Waffenbrüder',    minXp: 100, goldBonus: 15, essenzBonus: 3 },
    { level: 5,  title: 'Schicksalspaar',  minXp: 170, goldBonus: 20, essenzBonus: 4 },
    { level: 6,  title: 'Runengebunden',   minXp: 260, goldBonus: 30, essenzBonus: 5 },
    { level: 7,  title: 'Seelenband I',    minXp: 380, goldBonus: 40, essenzBonus: 6 },
    { level: 8,  title: 'Seelenband II',   minXp: 530, goldBonus: 50, essenzBonus: 8 },
    { level: 9,  title: 'Seelenband III',  minXp: 720, goldBonus: 60, essenzBonus: 10 },
    { level: 10, title: 'Ewiger Bund',     minXp: 999, goldBonus: 80, essenzBonus: 12 },
  ],
  roadmapData: [],
  users: {},
  playerProgress: {},
  ...
  socialData: { friendships: [], friendRequests: [], messages: [], trades: [], activityLog: [], swornBonds: [], challenges: [] },
...
function saveData() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.AGENTS, JSON.stringify(Object.values(state.store.agents)));
  } catch (e) {
...
function _saveQuestsImmediate() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.QUESTS, JSON.stringify(state.quests));
  } catch (e) {
...
function saveCampaigns() {
  debouncedSave('campaigns', () => {
    try {
      ensureRuntimeDir();
      atomicWriteSync(FILES.CAMPAIGNS, JSON.stringify(state.campaigns));
    } catch (e) {
...
function saveRituals() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.RITUALS, JSON.stringify({ rituals: state.rituals }));
...
function saveHabits() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.HABITS, JSON.stringify({ habits: state.habits }));
...
function saveNpcState() {
  try { atomicWriteSync(FILES.NPC_STATE, JSON.stringify(state.npcState)); } catch (e) { console.warn('[npc] Failed to save npcState:', e.message); }
}
...
function loadAppState() {
  try {
    if (fs.existsSync(FILES.APP_STATE)) {
      ...
      if (raw) {
        state.appState = { ...state.appState, ...raw };
        // Restore forgeFever from persisted appState
        if (raw.forgeFever) state.forgeFever = raw.forgeFever;
      }
    }
  } catch (e) { /* ignore */ }
}
...
function saveAppState() {
  // Include forgeFever in appState persistence
  const data = { ...state.appState, forgeFever: state.forgeFever || null };
  try { atomicWriteSync(FILES.APP_STATE, JSON.stringify(data)); } catch (e) { console.warn('[appState] Failed to save:', e.message); }
}
...
function saveFeedback() {
  try { atomicWriteSync(FILES.FEEDBACK, JSON.stringify(state.feedbackEntries)); } catch (e) { console.warn('[feedback] Failed to save:', e.message); }
}
...
function saveRotationState() {
  try { atomicWriteSync(FILES.ROTATION_STATE, JSON.stringify(state.rotationState)); } catch (e) { console.warn('[rotation] Failed to save rotationState:', e.message); }
}
...
function saveQuestCatalog() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.QUEST_CATALOG, JSON.stringify(state.questCatalog));
...
function saveClasses() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.CLASSES, JSON.stringify(state.classesData));
...
function saveRoadmap() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.ROADMAP, JSON.stringify(state.roadmapData));
...
function _saveUsersImmediate() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.USERS, JSON.stringify(state.users));
...
function _savePlayerProgressImmediate() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.PLAYER_PROGRESS, JSON.stringify(state.playerProgress));
...
function saveManagedKeys() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.KEYS, JSON.stringify(state.managedKeys));
  } catch (_) {}
}
...
function saveGachaState() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.GACHA_STATE, JSON.stringify(state.gachaState));
...
function _saveSocialImmediate() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.SOCIAL, JSON.stringify(state.socialData));
 ...
 * world_boss_spawn, world_boss_defeat, dungeon_complete, rift_complete, expedition_complete
 */
