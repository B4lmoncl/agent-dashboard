...
function saveData() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.AGENTS, JSON.stringify(Object.values(state.store.agents), null, 2));
  } catch (e) {
...
function saveCampaigns() {
  debouncedSave('campaigns', () => {
    try {
      ensureRuntimeDir();
      atomicWriteSync(FILES.CAMPAIGNS, JSON.stringify(state.campaigns, null, 2));
    } catch (e) {
...
function saveRituals() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.RITUALS, JSON.stringify({ rituals: state.rituals }, null, 2));
...
function saveHabits() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.HABITS, JSON.stringify({ habits: state.habits }, null, 2));
...
function saveNpcState() {
  try { atomicWriteSync(FILES.NPC_STATE, JSON.stringify(state.npcState, null, 2)); } catch (e) { console.warn('[npc] Failed to save npcState:', e.message); }
}
...
function saveAppState() {
  ...
  try { atomicWriteSync(FILES.APP_STATE, JSON.stringify(data, null, 2)); } catch (e) { console.warn('[appState] Failed to save:', e.message); }
}
...
function saveFeedback() {
  try { atomicWriteSync(FILES.FEEDBACK, JSON.stringify(state.feedbackEntries, null, 2)); } catch (e) { console.warn('[feedback] Failed to save:', e.message); }
}
...
function saveRotationState() {
  try { atomicWriteSync(FILES.ROTATION_STATE, JSON.stringify(state.rotationState, null, 2)); } catch (e) { console.warn('[rotation] Failed to save rotationState:', e.message); }
}
...
function saveQuestCatalog() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.QUEST_CATALOG, JSON.stringify(state.questCatalog, null, 2));
...
function saveClasses() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.CLASSES, JSON.stringify(state.classesData, null, 2));
...
function saveRoadmap() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.ROADMAP, JSON.stringify(state.roadmapData, null, 2));
...
function _savePlayerProgressImmediate() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.PLAYER_PROGRESS, JSON.stringify(state.playerProgress, null, 2));
...
function saveManagedKeys() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.KEYS, JSON.stringify(state.managedKeys, null, 2));
  } catch (_) {}
}
...
function saveGachaState() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.GACHA_STATE, JSON.stringify(state.gachaState, null, 2));
...
function _saveSocialImmediate() {
  try {
    ensureRuntimeDir();
    atomicWriteSync(FILES.SOCIAL, JSON.stringify(state.socialData, null, 2));
