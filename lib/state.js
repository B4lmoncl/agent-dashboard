...
const PLAYER_QUEST_TYPES = ['personal', 'learning', 'fitness', 'social', 'relationship-coop', 'companion', 'boss', 'development'];
...
const GOLD_BY_RARITY = _gameConfig.goldByRarity || {
  common: [5, 10], uncommon: [10, 18], rare: [18, 30], epic: [30, 50], legendary: [50, 80],
};
...
const RARITY_ORDER = _gameConfig.rarityOrder || ['common', 'uncommon', 'rare', 'epic', 'legendary', 'unique'];
...
function loadCampaigns() {
  ...
  rebuildCampaignsById();
}

function rebuildCampaignsById() {
  state.campaignsById = new Map();
  for (const c of state.campaigns) {
    if (c.id) state.campaignsById.set(c.id, c);
  }
}
...
function getActiveBuffs(userId) {
  ...
  const now = Date.now();
  const before = u.activeBuffs.length;
  ...
  // Persist if expired buffs were removed (prevents reappearing on restart)
  if (u.activeBuffs.length < before) saveUsers();
