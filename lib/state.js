...
const NPC_META = {
  dobbie:       { avatar: null, color: '#ff6b9d', role: 'Cat Overlord', description: 'Dobbie the demanding house cat. His requests are not optional. Resistance is futile.' },
  'npc-dobbie': { avatar: null, color: '#ff6b9d', role: 'Cat Overlord', description: 'Dobbie the demanding house cat. His requests are not optional. Resistance is futile.' },
};
...
const STREAK_MILESTONES = _gameConfig.streakMilestones || [
  ...
  { days: 14,  badge: '2W',      label: '2-Wochen',         xpBonus: 0,  lootTier: 'uncommon' },
  { days: 21,  badge: 'Silber',  label: 'Silber',           xpBonus: 10, lootTier: null },
  { days: 30,  badge: '1M',      label: 'Monat',            xpBonus: 0,  lootTier: 'rare' },
  { days: 60,  badge: 'Gold',    label: 'Gold',             xpBonus: 15, lootTier: null },
  { days: 90,  badge: 'Titan',   label: 'Unerschütterlich', xpBonus: 0,  lootTier: 'epic' },
...
const _shopData = tryLoadJsonSync(path.join(DATA_DIR, 'shopItems.json'), { items: [], gearTiers: [] });
...
function loadUsers() {
  ...
  for (const u of Object.values(state.users)) {
    ...
    // Ensure activeBuffs array exists
  ...
  try {
    if (fs.existsSync(FILES.ITEM_TEMPLATES)) {
      ...
      if (raw && Array.isArray(raw.items)) {
        ...
        for (const item of raw.items) {
          if (item.id) state.itemTemplates.set(item.id, item);
        }
        console.log(`[items] Loaded ${state.itemTemplates.size} item templates`);
      }
    }
...
function resolveItem(itemId) {
  ...
  // Direct lookup in item templates
  const tmpl = state.itemTemplates.get(itemId);
  if (tmpl) return tmpl;
  // Fallback: check FULL_GEAR_ITEMS (for backward compat with shop gear)
  const gear = state.FULL_GEAR_ITEMS.find(g => g.id === itemId);
