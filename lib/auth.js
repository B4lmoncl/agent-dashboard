...
// Persisted via state.revokedTokens to survive server restarts.
// Map<jti, expiresAt> — auto-prunes expired entries to prevent memory leak.
function _getRevokedMap() {
  if (!state._revokedTokensMap) {
    state._revokedTokensMap = new Map(Object.entries(state.appState?.revokedTokens || {}));
  }
  return state._revokedTokensMap;
}
function _persistRevoked() {
  const map = _getRevokedMap();
  state.appState = state.appState || {};
  state.appState.revokedTokens = Object.fromEntries(map);
  try { saveAppState(); } catch { /* ignore */ }
}
...
function pruneRevokedTokens() {
  ...
  const map = _getRevokedMap();
  let pruned = false;
  for (const [jti, expiresAt] of map) {
    if (expiresAt <= now) { map.delete(jti); pruned = true; }
  }
  if (pruned) _persistRevoked();
}
...
function verifyRefreshToken(token) {
  try {
    const decoded = jwt.verify(token, getRefreshSecret(), { algorithms: ['HS256'] });
    if (decoded.type !== 'refresh') return null;
    if (!decoded.jti) return null; // reject tokens without JTI
    pruneRevokedTokens();
    if (_getRevokedMap().has(decoded.jti)) return null;
    return decoded;
  } catch {
    return null;
  }
...
function revokeRefreshToken(token) {
  try {
    const decoded = jwt.verify(token, getRefreshSecret(), { algorithms: ['HS256'], ignoreExpiration: true });
    if (decoded.jti) {
      const expiresAt = decoded.exp ? decoded.exp * 1000 : Date.now() + REFRESH_COOKIE_MAX_AGE;
      _getRevokedMap().set(decoded.jti, expiresAt);
      _persistRevoked();
    }
...
function resolveAuth(req) {
  ...
  if (apiKey && state.validApiKeys && state.validApiKeys.has(apiKey)) {
    ...
    // Timing-safe comparison — pad to equal length to avoid leaking key length
    let isAdmin = false;
    if (apiKey && master) {
      const keyBuf = Buffer.from(apiKey);
      const masterBuf = Buffer.from(master);
      // Pad shorter buffer to prevent length leak via early exit
      const maxLen = Math.max(keyBuf.length, masterBuf.length);
      const paddedKey = Buffer.alloc(maxLen);
      const paddedMaster = Buffer.alloc(maxLen);
      keyBuf.copy(paddedKey);
      masterBuf.copy(paddedMaster);
      isAdmin = keyBuf.length === masterBuf.length && crypto.timingSafeEqual(paddedKey, paddedMaster);
    }
    return {
